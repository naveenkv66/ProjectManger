...
namespace ProjectManager.BusinessLayer
{
    public class ProjectBL : IProjectBL
    {
        ...
        public Collection<CommonEntities.Projects> GetProjects()
        {
            ...
            _projectManager.Projects1.SelectMany
            (
                proj => _projectManager.Users1.Where(user => proj.ProjectID == user.ProjectID).DefaultIfEmpty(),
                (x, y) => new
                {
                    Projects = x,
                    Users = y
                ...
                .ForEach(y => projCollection.Add(
                    new CommonEntities.Projects
                    {
                        ...
                        NoofTasks = _projectManager.Tasks1.Where(x => x.ProjectID == y.Projects.ProjectID).Count(),
                        NoofCompletedTasks = _projectManager.Tasks1.Where(x => x.ProjectID == y.Projects.ProjectID && x.Status == true).Count(),
        ...
        public void AddProject(CommonEntities.Projects project)
        {
            ...
            _projectManager.Projects1.Add(proj);
            _projectManager.SaveChanges();
            var proId = proj.ProjectID;
            var ur = _projectManager.Users1.Where(x => x.UserID == project.ManagerID).FirstOrDefault();
            if (ur != null)
            {
                ur.ProjectID = proId;
                _projectManager.SaveChanges();
        ...
        {
            var proj = _projectManager.Projects1.Where(x => x.ProjectID == project.ProjectID).FirstOrDefault();
            var user = _projectManager.Users1.Where(x => x.UserID == project.ManagerID).FirstOrDefault();
            var extUser = _projectManager.Users1.Where(x => x.ProjectID == project.ProjectID).FirstOrDefault();
